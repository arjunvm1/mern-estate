...
export default function Profile() {
  ...
  const [updateSuccess, setUpdateSeccess] = useState(false)
  ...
  const handleFileUpload = (file) => {
    ...
    uploadTask.on(
      ...
      () => {
        // Handle when the file finishes uploading
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <p className="text-sm self-center">{fileUploadError ? <span className="text-red-700">Error Image Upload (image must be less tha 2 MB)</span> : filePerc > 0 && filePerc < 100 ? <span className="text-slate-700">{`Uploading ${filePerc}%...`}</span> : filePerc === 100 ? <span className="text-green-700">Successfully Uploaded!</span> : ""}</p>
      ...
      {/* display listings if the length > 0 */}
      {userListings &&
